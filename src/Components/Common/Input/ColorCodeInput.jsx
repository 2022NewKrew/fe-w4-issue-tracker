 ...
 * @constructor
 */
const ColorCodeInput = ({ isDisabled, onChangeListener }) => {
  // input 태그의 값을 바꿔야할때만 수정
  const [inputValue, setInputValue] = useState('#')
  // input 태그의 값과 일치
  const [colorCode, setColorCode] = useState('')

  const setColorCodeWithListener = useCallback(
    (value) => {
      setColorCode(value)
  ...
  const generateColorCode = useCallback(() => {
    ...
    const newColorCode =
      '#' + colorValue.toString(16).toUpperCase().padStart(6, '0')
  ...
  return (
    <TextInput
      ...
      <IconBox onClick={generateColorCode}>
        <RefreshIcon width="1em" height="1em" />
      </IconBox>
    </TextInput>
  )
}
